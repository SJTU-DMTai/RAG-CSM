...
class Dataset:
    ...
    def __init__(
        ...
        self.trunc_idx = config['trunc_idx'] if 'trunc_idx' in config else None
    ...
    def _load_data(self, dataset_name: str, dataset_path: str) -> List[Item]:
        ...
        if self.trunc_idx is not None:
            data = data[: self.trunc_idx]
    ...
    def question(self) -> List[Optional[str]]:
        if self.dataset_name == 'truthful_qa':
            return [item.question + '\n' + '\n'.join(item.choices) for item in self.data]
        else:
