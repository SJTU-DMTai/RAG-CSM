...
class SequentialPipeline(BasicPipeline):
    def __init__(self, config, prompt_template=None, retriever=None, generator=None):
        ...
        self.refiner_train = config['refiner_train']
        self.refiner_selector = config['refiner_selector']
        self.save_retrieval_cache = config['save_retrieval_cache']
        if (self.refiner_train and 'sft' in self.refiner_selector) or self.save_retrieval_cache:
            self.generator = None
        else:
        ...
        if config["refiner_name"] is not None:
            self.refiner = get_refiner(
                config, generator=self.generator, prompt_template=self.prompt_template)
            self.refiner_labeling = config['refiner_labeling']
            self.refiner_use_selector = config['use_selector']
        else:
            self.refiner = None
        self.trunc_idx = config['trunc_idx']
    ...
    def run(self, dataset, do_eval=True, pred_process_fun=None):
        ...
        if self.config['dataset_name'] == 'truthful_qa':
            input_query = [s.split('\n')[0] for s in input_query]
        ...
        if self.refiner:
            ...
            elif "loo" in self.refiner.name or "cxmi" in self.refiner.name:
                if self.refiner_train:
                    input_prompts = self.refiner.train_selector(
                        dataset, refiner_selector=self.refiner_selector, trunc_idx=self.trunc_idx)
                    return
                refine_results = self.refiner.batch_run(
                    dataset, use_selector=self.refiner_use_selector, topk=self.topk)
                if self.refiner_labeling:
                    return
                input_prompts = [
                    self.prompt_template.get_string(question=q, formatted_reference=r)
                    for q, r in zip(dataset.question, refine_results)
                ]
            # elif "random" in self.refiner.name:
            #     import torch
            #     # randomly select topk docs from retrieval results
            #     select_idx = torch.randint(0, len(retrieval_results[0]), (self.topk,))
            #     refine_results = []
            #     for i in range(len(retrieval_results)):
            #         tmp = ''
            #         for j in select_idx:
            #             tmp += retrieval_results[i][j]['contents'] + '\n'
            #         refine_results.append(tmp)
            #     input_prompts = [
            #         self.prompt_template.get_string(question=q, formatted_reference=r)
            #         for q, r in zip(dataset.question, refine_results)
            #     ]
        ...
        if self.refiner and not (self.config['refiner_name'] == 'rankgpt' or self.config['refiner_name'] == 'generator'):
            del self.refiner
        pred_answer_list = self.generator.generate(input_prompts)
        dataset.update_output("pred", pred_answer_list)
