...
class HFCausalLMGenerator(BaseGenerator):
    ...
    def __init__(self, config, model=None):
        super().__init__(config)
        self.use_custom_llm = self._config["use_custom_llm"]
    ...
    def _load_model(self, model=None):
        ...
        if model is None:
            ...
            if self.use_custom_llm:
                from flashrag.generator.custom_llm import CustomLlamaForCausalLM
    ...
    def generate(
        ...
        **params,
        ...
        import torch
        if do_generate:
            iterator = tqdm(range(0, len(input_list), batch_size), desc="Generation process: ")
        else:
            iterator = range(0, len(input_list), batch_size)
        for idx in iterator:
            with torch.inference_mode():
    ...
    # def cal_pred_loss(self, prompts, labels, batch_size=2):
    #     loss = []
    #     for i in range(0, len(prompts), batch_size):
    #         batch_prompts = prompts[i: i + batch_size]
    #         batch_labels = labels[i: i + batch_size]
    #         batch_label_texts = [l[0] for l in batch_labels]
    #         batch_prompts = [p+l for p, l in zip(batch_prompts, batch_label_texts)]
    #         all_tokens = self.tokenizer(batch_prompts, return_tensors='pt', padding=True).to(self.device)
    #         target_tokens = self.tokenizer(batch_label_texts)
    #         output_mask = target_tokens.attention_mask
    #         batch_target = deepcopy(all_tokens.input_ids)
    #         for j in range(len(batch_prompts)):
    #             batch_target[j, :-len(output_mask[j])] = -100
    #         # outputs = self.model(**all_tokens, labels=target)
    #         # loss = outputs.loss
    #         logits = self.model(**all_tokens).logits
    #         for j in range(len(batch_prompts)):
    #             loss.append(self._loss_fn(logits[j], batch_target[j]))
    #     return loss

    @torch.no_grad()
    def relocate_chunk_index(self, input_text, offset_mapping, context_spliter='\n'):
        # re-locate the sentence index in the tokenized encoding
        offset_mapping = offset_mapping.cpu().numpy()
        chunk_indicies = []
        for i, pos in enumerate(offset_mapping):
            phrase = input_text[pos[0]: pos[1]]
            if phrase.strip() == 'Doc' and len(chunk_indicies) == 0:
                chunk_indicies.append(i)
            if len(chunk_indicies) != 0:
                if phrase.replace(' ', '') == context_spliter:
                    chunk_indicies.append(i+1)
                if phrase == self.split_token:
                    chunk_indicies.append(i-2)
                    break
        return chunk_indicies

    def cal_pred_loss(self, prompts, labels, batch_size=1):
        total_loss = []
        for i in range(len(labels)):
            labels[i] = [str(label) for label in labels[i]]
        for prompt_idx in range(len(prompts)):
            prompt = prompts[prompt_idx]
            candidate_labels = labels[prompt_idx]
            sample_losses = []  # loss of each candidate answer
            input_texts = [prompt + str(label) for label in candidate_labels]

            for i in range(0, len(input_texts), batch_size):
                batch_inputs = input_texts[i:i + batch_size]
                batch_labels = candidate_labels[i:i + batch_size]
                all_tokens = self.tokenizer(batch_inputs, return_tensors='pt', padding=True).to(self.device)
                target_tokens = self.tokenizer(batch_labels, return_tensors='pt', padding=True)
                output_mask = target_tokens.attention_mask
                batch_target = deepcopy(all_tokens.input_ids)
                for j in range(len(batch_inputs)):
                    batch_target[j, :-len(output_mask[j])] = -100
                logits = self.model.forward(**all_tokens).logits
                for j in range(len(batch_inputs)):
                    loss = self._loss_fn(logits[j], batch_target[j])
                    sample_losses.append(loss.item())
            total_loss.append(min(sample_losses))
        return total_loss

    def cal_pred_loss_w_context(self, prompts, labels, context_scores, batch_size=2):
        total_loss = torch.tensor([], device=self.device)
        for prompt_idx in range(len(prompts)):
            prompt = prompts[prompt_idx]
            candidate_labels = labels[prompt_idx]
            if context_scores is not None:
                context_score = context_scores[prompt_idx]
            sample_losses = []  # loss of each candidate answer
            input_texts = [prompt + str(label) for label in candidate_labels]
            ...
            for i in range(0, len(input_texts), batch_size):
                ...
                embs = emb_layer(all_tokens.input_ids) # (batch_size, seq_length, hidden_size)
                emb_scores = torch.ones(embs.shape, device=self.device, dtype=torch.bfloat16)
                # broadcast context_score to all_tokens
                for j in range(len(batch_inputs)):
                    context_indicies = self.relocate_chunk_index(batch_inputs[j], all_tokens.offset_mapping[j])
                    for k in range(len(context_indicies) - 1):
                        emb_scores[j][context_indicies[k]: context_indicies[k+1]] = context_score[k]
                target_tokens = self.tokenizer(batch_labels)
                output_mask = target_tokens.attention_mask
                batch_target = deepcopy(all_tokens.input_ids)
                for j in range(len(batch_inputs)):
                    batch_target[j, :-len(output_mask[j])] = -100
                logits = self.model.forward(
                    ...
                    input_scores=emb_scores
                ).logits
                for j in range(len(batch_inputs)):
                    loss = self._loss_fn(logits[j], batch_target[j])
                    sample_losses.append(loss)
                    # sample_losses = torch.cat([sample_losses, loss])
            # total_loss.append(min(sample_losses))
            total_loss = torch.cat([total_loss, min(sample_losses).view(1)])
        return total_loss
